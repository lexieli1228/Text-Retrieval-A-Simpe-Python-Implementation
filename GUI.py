...
class MainPanel:
    ...
    def search_request(self, terms):
        ...
        json_string = json.dumps(terms)
        client.send(bytes(json_string.encode('utf-8')))
        print("client successful send message")
        
        # 之前是2048
        # 现在改成10000
        json_re = client.recv(100000)
        result = json.loads(json_re.decode('utf-8'))
        print("client successful receive message")
        
        result_documents = []
        if len(result) == 1:
            if result[0][0] == -1:
                result_documents.append(('error', result[0][1]))

        for i in range(len(result)):
            result_documents.append((result[i][0], result[i][1]))
        
        self.documents = result_documents
