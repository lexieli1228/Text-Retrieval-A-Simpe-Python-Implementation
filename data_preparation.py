...
# overall tf-df value and vector calculation for the body part
...
# calculate tf value for the title part
def get_tf_title(x):
    t = dict(x['word title'].value_counts()/x['word title'].value_counts().sum())
    return t


# calculate idf value for the title part
def get_idf_title(x, total_num):
    return np.log(total_num/x['id'].nunique())


# form tf-idf vector for the title part
...
# overall tf-df value and vector calculation for the title part
...
# calculate and store synonym words
